...
can hold a word, not 8-bit byte.
Once read into a register, a word can be accessed with 4 fields,
...
`CAR` and `CDR` came from _contends_of_address_part_of_register_ and
_contents_of_decrement_part_of_register.  
I suspect that this architecture of IBM704
influenced the design of LISP using two-pointer cells (_cons_cells_)
...
cells are called _full_word_.  A character string is represented
...
splitting memory into region; certain region only stores cons cells,
