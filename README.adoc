...
into Gauche, however.  Treating a list starting with `LAMBDA` is a
feature of `EVAL`, not Gauche.  We have to make `EVAL` understand
...
Alternatively, run `./configure` then `make` in the toplevel source dirctory.
