...
Run `mexpr-env.scm` with `-e` option, and save the result in `eval-star.lisp`:

[source, console]
----
$ gosh tools/mexpr-env.scm -e examples/eval.mx > eval-star.lisp
---

We use suffix `lisp` since but it's arbitrary.  The created `eval-star.lisp`
...
It can be loaded to `gosh` after using `LISP1.5.axioms`.  
...
inner `EVAL` is interpreted by the outer `EVAL`.  And the
expression `(CONS (QUOTE X) (QUOTE Y))` is interpreted by
the inner `EVAL`:
...
For example, edit the `eval-star.lisp` created by `mexpr-env.scm` above
...
`KWOTE`s are recognized by the inner `EVAL`.
